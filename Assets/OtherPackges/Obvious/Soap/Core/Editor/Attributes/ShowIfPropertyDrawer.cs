            ...
            switch (conditionField.propertyType)
            {
                ...
                case SerializedPropertyType.Enum:
                    try
                    {
                        ...
                        {
                            ShowError(position, label, "Comparison value is required for enum types");
                            return;
                        }
                        
                        int enumValue = conditionField.enumValueIndex;
                        int comparisonEnumValue = (int)attribute.comparisonValue;
                        _showField = enumValue == comparisonEnumValue;
                    }
                    catch
                    {
                        ShowError(position, label, "Invalid comparison Value Type for enum");
                        return;
                    }
                    break;
                default:
