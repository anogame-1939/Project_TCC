    ...
    public partial class MB3_MeshCombinerSingle : MB3_MeshCombiner
    {
        ...
        */
        ...
        public bool InstanceID2DGO(int instanceID, out MB_DynamicGameObject dgoGameObject)
        {
            ...
            Debug.LogError("Could not find a cached game object matching InstanceID: " + instanceID);
        ...
        {
            if (textureBakeResults.resultType == MB2_TextureBakeResults.ResultType.textureArray)
            {
        ...
        public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
        {
            ...
            if (validationLevel > MB2_ValidationLevel.none)
            {
                ...
                {
                    // correct cache IDs in case Unity has updated the gameobject IDs
                    // check if our deleteIDs match up with IDs in cache
                    bool deleteIDsMatchCache = true;
                    HashSet<int> deleteSet = new HashSet<int>(deleteGOinstanceIDs);
                    for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
                    {
                        if (!deleteSet.Contains(mbDynamicObjectsInCombinedMesh[i].instanceID))
                        {
                            deleteIDsMatchCache = false;
                            break;
                        }
                    }

                    // if delete IDs do not match cache update the cache ids directly from the game objects
                    if (!deleteIDsMatchCache)
                    {
                        for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
                        {
                            if (mbDynamicObjectsInCombinedMesh[i].gameObject == null)
                            {
                                Debug.LogError("An instanceID to be deleted does not match any of the cached instanceIDs from the bake and the " +
                                               " corresponding source game object has already been deleted." +
                                               " This can happen if objects were baked, then the scene was saved, closed, opened and a delete bake is attempted." +
                                               " Try deleting a source object from the baked mesh by passing in the source game object instead of the instance ID");
                                return false;
                            }
                            else
                            {
                                mbDynamicObjectsInCombinedMesh[i].instanceID = mbDynamicObjectsInCombinedMesh[i].gameObject.GetInstanceID();
                            }
                        }
                    }
