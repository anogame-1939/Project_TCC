...
namespace DigitalOpus.MB.Core {
    ...
    public abstract class MB3_MeshCombiner : MB_IMeshBakerSettings, IDisposable
    {
        ...
        [SerializeField] protected MeshCombiningStatus _bakeStatus = MeshCombiningStatus.preAddDeleteOrUpdate;

        public virtual MeshCombiningStatus bakeStatus
        {
            get { return _bakeStatus; }
        }
...
#else
        [SerializeField]
        protected MB_MeshCombineAPIType _meshAPItoUse = MB_MeshCombineAPIType.simpleMeshAPI;
#endif
...
#else
                if (_meshAPItoUse == MB_MeshCombineAPIType.betaNativeArrayAPI)
                {
                    Debug.LogError("Switch MeshBaker->Settings->Mesh API to 'Simple Mesh API'. Native Array API for meshes is only available in Unity versions 2020.2 and newer. Using Simple Mesh API");
                }

                return MB_MeshCombineAPIType.simpleMeshAPI;
#endif
            }
            set { _meshAPItoUse = value; }
        }
