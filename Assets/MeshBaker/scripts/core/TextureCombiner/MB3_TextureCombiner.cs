...
namespace DigitalOpus.MB.Core
{
    ...
    public class ShaderTextureProperty
    {
        ...
        /// <summary>
        /// Albedo, specGloss, emmission etc... textures assgned to shaders will be will usually be 
        /// gamma corrected (sRGB == true).
        /// This is true even when a project is configured PlayerSettings.colorSpace = linear.
        /// Normal maps should always be "linear" evein when project color space is gamma.
        /// Other textures it is hard to tell. SplatMaps, MaskMaps, DepthMaps should probably be linear but it
        /// depends on how the textures are authored and used in the shader.
        /// 
        /// Mesh Baker will configure the generated atlas texture for this shader property  atlas.sRGB = isGammaCorrected.
        /// Mesh Baker also excpects source textures to for this map to be gamma corrected.
        /// </summary>
    ...
    public class MB3_TextureCombiner
    {
        ...
        public static Color NEUTRAL_NORMAL_MAP_COLOR_SWIZZLED = new Color(1.0f, .5f, .5f, .5f);

        public static Color NEUTRAL_NORMAL_MAP_COLOR_NON_SWIZZLED = new Color(.5f, .5f, 1f, .5f);
        ...
        *  <remarks>Combines meshes and generates texture atlases</remarks> */
        ...
        {
            Texture2D tx = MB_Utility.createTextureCopy(t, propertyName.isGammaCorrected);
            tx.name = string.Format("tmpCopy{0}_{1}x{2}", _temporaryTextures.Count, tx.width, tx.height);
            TemporaryTexture txx = new TemporaryTexture(propertyName.name, tx);
            _temporaryTextures.Add(txx);
            return tx;
        }
        ...
        {
            Texture2D tx = MB_Utility.resampleTexture(t, propertyName.isGammaCorrected, w, h);
        ...
        */
        ...
        public static bool ShouldTextureBeLinear(ShaderTextureProperty shaderTextureProperty)
        {
            if (shaderTextureProperty.isNormalMap ||
                !shaderTextureProperty.isGammaCorrected) return true;
            else return false;
        }
