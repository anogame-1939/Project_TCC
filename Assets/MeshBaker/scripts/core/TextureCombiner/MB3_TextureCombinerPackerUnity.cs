    ...
    internal class MB3_TextureCombinerPackerUnity : MB3_TextureCombinerPackerRoot
    {
                ...
                else
                {
                    ...
                    for (int texSetIdx = 0; texSetIdx < data.distinctMaterialTextures.Count; texSetIdx++)
                    {
                        ...
                        if (tx.width != tWidth || tx.height != tHeight)
                        {
                            ...
                            tx = combiner._resizeTexture(prop, (Texture2D)tx, tWidth, tHeight);
                        }
                        ...
                        if (data._considerNonTextureProperties)
                        {
                            ...
                            tx = combiner._createTextureCopy(prop, tx);
                    ...
                    else
                    {
                        ...
                        atlas = _copyTexturesIntoAtlas(prop, texToPack, data._atlasPadding_pix, uvRects, atlasSizeX, atlasSizeY, combiner);
                    }
                }
        ...
        internal static Texture2D _copyTexturesIntoAtlas(ShaderTextureProperty prop, Texture2D[] texToPack, int padding, Rect[] rs, int w, int h, MB3_TextureCombiner combiner)
        {
            ...
            for (int i = 0; i < rs.Length; i++)
            {
                ...
                {
                    tmpTex = t = MB_Utility.resampleTexture(t, prop.isGammaCorrected, ww, hh);
                }
