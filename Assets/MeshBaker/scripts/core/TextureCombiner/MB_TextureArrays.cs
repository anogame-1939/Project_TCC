    ...
    public class MB_TextureArrays
    {
            ...
            bool[] hasTexForProperty, MB3_TextureCombiner combiner, MB2_LogLevel LOG_LEVEL)
        {
            ...
            for (int propIdx = 0; propIdx < texPropertyNames.Length; propIdx++)
            {
                ...
                Debug.Assert(masterListOfTexProperties.Count == texPropertyNames.Length && masterListOfTexProperties[propIdx].name.Equals(texPropertyNames[propIdx]), "");
                ...
                bool isLinear = MB3_TextureCombiner.ShouldTextureBeLinear(masterListOfTexProperties[propIdx]);
                ...
                for (int sliceIdx = 0; sliceIdx < numSlices; sliceIdx++)
                {
                    ...
                    if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.LogFormat("Slice: {0}  texture: {1}  w: {2}  h: {3}  prop: {4}", sliceIdx, srcTex, srcTex.width, srcTex.height, texPropertyNames[propIdx]);
                    bool isCopy = false;
                    if (srcTex == null)
                    {
            ...
            for (int propIdx = 0; propIdx < hasTexForProperty.Length; propIdx++)
            {
                ...
                for (int sliceIdx = 0; sliceIdx < numSlices; sliceIdx++)
                {
                    ...
                    if (sliceTex != null)
                    {
                        ...
                        if (logLevel >= MB2_LogLevel.trace) Debug.Log("Considering format of property:" + textureShaderProperties[propIdx].name + " texture: '" + sliceTex.name + "' format:" + sliceTex.format);
                        ...
                        else
                        {
                            ...
                            if ((sliceTex.width != targetWidth || 
                                sliceTex.height != targetHeight) ||
                                (!isAsset && sliceTex.format != format))
                            {
                                ...
                                resultAtlasesAndRectSlices[sliceIdx].atlases[propIdx] = textureEditorMethods.CreateTemporaryAssetCopyForTextureArray(textureShaderProperties[propIdx], sliceTex, targetWidth, targetHeight, format, logLevel);
                            ...
                            float maxTimePerFrame = .01f)
        {
            ...
            if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("=== STAGE 1   Baking atlases for result material " + resMatIdx + " num slices:" + resMatConfig.slices.Count);
                    ...
                    // Track which slices are new generated texture instances. Atlases could be original texture assets (one tex per atlas) or temporary texture instances in memory that will need to be destroyed.
                    {
                        for (int texPropIdx = 0; texPropIdx < sliceAtlasesAndRectOutput.atlases.Length; texPropIdx++)
                        {
                            ...
                            if (atlas != null)
                            {
                                ...
                                if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("Property: " + sliceAtlasesAndRectOutput.texPropertyNames[texPropIdx]
                                                + " atlasWasSrcTex:" + atlasWasASourceTexture);
                            }
            ...
            if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("=== STAGE 2 Generate Temporary Textures");
            ...
            try
            {
                ...
                for (int texFormatSetIdx = 0; texFormatSetIdx < textureArrayOutputFormats.Length; texFormatSetIdx++)
                {
                    ...
                    if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("=== STAGE 3 formatSet: " + textureArrayFormatSet.name + " generate necessary temporary textures ");
                            ...
                            if (hasTexForProperty[propIdx])
                            {
                                ...
                                for (int sliceIdx = 0; sliceIdx < numSlices; sliceIdx++)
                                {
                                    if (bakedMatsAndSlicesResMat.slices[sliceIdx].atlases[propIdx] == null)
                                    {
                                        ...
                                        ShaderTextureProperty prop = texPropertyNames[propIdx];
                                        Color col;
                                        if (prop.isNormalMap)
                                        {
                                            col = MB3_TextureCombiner.NEUTRAL_NORMAL_MAP_COLOR_NON_SWIZZLED;
                                        } else
                                        {
                                            col = textureBlender.GetColorForTemporaryTexture(resMatConfig.slices[sliceIdx].sourceMaterials[0].sourceMaterial, prop);
                                        }
                                        MB_Utility.setSolidColor(sliceTex, col);
                                        // Now create a copy of this texture in target format.
                                        bakedMatsAndSlicesResMat.slices[sliceIdx].atlases[propIdx] = editorMethods.CreateTemporaryAssetCopyForTextureArray(texPropertyNames[propIdx], sliceTex, targetWidth, targetHeight, format, LOG_LEVEL);
                    ...
                    if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("=== STAGE 4 formatSet: " + textureArrayFormatSet.name + " Converting source textures to readable formats.");
                    if (MB_TextureArrays.ConvertTexturesToReadableFormat(texPropertyData, bakedMatsAndSlicesResMat.slices, hasTexForProperty, texPropertyNames, combiner, LOG_LEVEL, temporaryTextureAssets, editorMethods))
                    {
                        ...
                        if (LOG_LEVEL >= MB2_LogLevel.debug) Debug.Log("=== STAGE 5 formatSet: " + textureArrayFormatSet.name + " Creating texture array");
