...
namespace DigitalOpus.MB.Core
{
    public class MB3_TextureCombinerNonTextureProperties
    {
        ...
        class NonTexturePropertiesDontBlendProps : NonTextureProperties
        {
        ...
        private void FindBestTextureBlender(Material resultMaterial)
        {
            ...
            if (resultMaterialTextureBlender is TextureBlenderFallback)
            {
                if (LOG_LEVEL >= MB2_LogLevel.error) Debug.LogWarning("Using _considerNonTextureProperties could not find a TextureBlender that matches the shader on the result material (" + resultMaterial.shader.name + "). Using the Fallback Texture Blender.");
            }
