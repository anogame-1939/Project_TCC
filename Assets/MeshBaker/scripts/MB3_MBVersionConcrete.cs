...
namespace DigitalOpus.MB.Core
{
    public class MBVersionConcrete : MBVersionInterface
    {
        ...
        public bool IsSwizzledNormalMapPlatform()
        {
            ...
            isSwizzled = !GraphicsSettings.HasShaderDefine(BuiltinShaderDefine.UNITY_NO_DXT5nm);
            if (Application.isEditor)
            {
#if UNITY_EDITOR
#if UNITY_2023_1_OR_NEWER
                // do nothing our isSwizzled test works correctly
#else
                // This is horrible workaround for Unity bug. 
                // When the build platform is Mobile, UnityEditor is missing the define: UNITY_NO_DXT5nm
                // The define should be there. The normal textures have been imported without swizzle
                var bt = EditorUserBuildSettings.activeBuildTarget;

                // Not sure which other platforms should have this.   'Nintendo Switch?
                bool isMobileBuildTarget = bt == BuildTarget.Android ||
                                            bt == BuildTarget.iOS;
