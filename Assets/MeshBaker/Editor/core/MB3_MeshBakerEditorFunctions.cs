...
public class MB3_MeshBakerEditorFunctions
{
    ...
    public static string SanitizeAndMakeFullPathRelativeToAssetsFolderAndValidate(string path, out string errorMessage, out bool isSuccess)
    {
        if (path == null)
        {
            isSuccess = false;
            errorMessage = "Output path cannot be null. Setting path to Assets";
            return "Assets";
        }
        path = path.Replace("\\", "/");
        if (path.Equals("Assets") || path.StartsWith("Assets/"))
        {
            isSuccess = true;
        ...
        {
            string projectNamePlusAssetsFolder;
            {
                string fullAssetFolderPath = Application.dataPath.Replace("\\", "/");
                int lastSlashIndex = fullAssetFolderPath.LastIndexOf('/');
                int secondToLastSlashIndex = fullAssetFolderPath.LastIndexOf('/', lastSlashIndex - 1);
                projectNamePlusAssetsFolder = fullAssetFolderPath.Substring(secondToLastSlashIndex + 1);
            }
            if (path.Contains(projectNamePlusAssetsFolder)) 
            {
                int indexOfStartOfFinalRelativePathFromAssetsFolder = path.LastIndexOf(projectNamePlusAssetsFolder) + projectNamePlusAssetsFolder.Length
                                                                      - "Assets".Length;
                string finalRelativePathFromAssetsFolder = path.Substring(indexOfStartOfFinalRelativePathFromAssetsFolder);
                isSuccess = true;
                errorMessage = "No Error";
                path = finalRelativePathFromAssetsFolder;
            } else
            {
                isSuccess = false;
                errorMessage = "Output folder must be in the project Assets folder.";
            }
        }
        if (isSuccess)
        {
            string gid = AssetDatabase.AssetPathToGUID(path);
            if (gid == null || gid.Length == 0)
            {
                isSuccess = false;
                errorMessage = "Output folder must be an existing folder in the Unity project Assets folder";
            }
        }
        return path;
    }
