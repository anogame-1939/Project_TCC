    ...
    public class MB3_EditorMethods : MB2_EditorMethodsInterface
    {
        ...
        private void _SetTextureFormat_DefaultPlatform(Texture2D tx, TextureFormatInfo_AbstractDefaultPlatform toThisFormat, bool addToList, bool setNormalMap)
        {
            ...
            if (ai != null && ai is UnityEditor.TextureImporter)
            {
                ...
                bool doImport = false;
                if (MBVersion.Is_2017_1_OrNewer())
                {
        ...
        public void SaveAtlasToAssetDatabase(Texture2D atlas, ShaderTextureProperty texPropertyName, int atlasNum, bool doAnySrcMatsHaveProperty, Material resMat)
        {
            ...
            else
            {
                ...
                if (ConstructFilename(resMat, texPropertyName.name, "atlas", "", atlasNum, out pth, out relativePath))
                {
                    ...
                    Texture2D newTex = MB_Utility.createTextureCopy(atlas, texPropertyName.isGammaCorrected);
        ...
        public Texture2D CreateTemporaryAssetCopyForTextureArray(ShaderTextureProperty shaderProp, Texture2D sliceTex, int w, int h, TextureFormat format, MB2_LogLevel logLevel)
        {
            bool foundMatch;
            ...
            string tryPth = workingFolder + "/" + shaderProp.name + "_" + sliceTex.name + "_TEMP.png";
            ...
            // Duplicate the source texture and save it as a truecolor temporary asset
            {
                ...
                // Resize it.
                Texture2D newTex = MB_Utility.resampleTexture(newTex1, shaderProp.isGammaCorrected, w, h);
            ...
            if (settings.format != targetImporterFormat)
            {
                ...
                int maxTexSize = Mathf.NextPowerOfTwo(Mathf.Max(w, h));
                settings.maxTextureSize = maxTexSize;
                ai.SetPlatformTextureSettings(settings);
            }

            if (logLevel >= MB2_LogLevel.debug) Debug.LogFormat("Creating temporary texuture asset to resize property: {7} srcTexture: {0} w:{1} h:{2} format:{3} TO w:{4} h:{5} format:{6}  path:{8}",
                temporaryTex.name, temporaryTex.width, temporaryTex.height, temporaryTex.format, w, h, format, shaderProp.name, shortPath);
            ai.SaveAndReimport();
