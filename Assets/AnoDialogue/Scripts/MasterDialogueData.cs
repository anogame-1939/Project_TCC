...
namespace AnoGame.AnoDialogue
{
    ...
    public class MasterDialogueData : ScriptableObject
    {
        ...
        /// <summary>
        /// Default Episode (-1,-1,-1) を (0,0,0) に移行する。
        /// </summary>
        [ContextMenu("Migrate Default IDs: -1 to 0")]
        public void MigrateDefaultEpisodeToZero()
        {
            int migratedEpisode = 0;
            ...
            Debug.Log($"[MigrateDefaultIDs] 完了: Ep→0: {migratedEpisode}, Ch→0: {migratedChapter}, Sec→0: {migratedSection} / Total: {Conversations.Count}");
            UnityEditor.EditorUtility.SetDirty(this);
        }
#endif
    }
